...
void SaveShaderCompilationTime(std::wstring shaderPath)
{
	...
	auto modifiedTime = (long long) result.st_mtime;
...
bool SaveBlobToDisk(std::wstring shaderPath, SIZE_T size, uint32_t* buffer)
{
	...
	std::ofstream myFile(compiledShaderPath, std::ifstream::in | std::ifstream::out | std::ifstream::trunc | std::ios::binary);
...
loadedBlob LoadBlobFromDisk(std::wstring shaderPath)
{
	...
	if (!myFile.is_open())
	{
	 ...
	 std::unique_ptr<uint32_t[]>buffer = std::unique_ptr<uint32_t[]>(new uint32_t[size]);
...
void ShaderLoader::shaderCompile(std::wstring shaderFilename, bool is_frag)
{
	...
	auto utf = library->GetBlobAsUtf8(code.p, DXC_CP_ACP);

	result->GetResult(&code);

	SaveBlobToDisk(filename + (is_frag ? L".frag" : L".vert"),code->GetBufferSize(), (uint32_t*)code->GetBufferPointer());
}
